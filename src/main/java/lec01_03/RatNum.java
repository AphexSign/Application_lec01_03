...
/** <b>RatNum</b> представляет собой <b>неизменяемое</b> рациональное число.
 Он включает в себя все элементы в наборе рациональных чисел, а
 также специальный элемент "NaN" (не являющийся числом), который получается в результате
 деления на ноль.
 <p>
 Элемент "NaN" является особенным во многих отношениях. Любая арифметическая
 операция (например, сложение), включающая "NaN", вернет "NaN".
 Что касается операций сравнения, таких как меньше, чем, "NaN"
 считается равным самому себе и больше, чем все остальные рациональные числа.
 <p>
 Примеры RatNums включают "-1/13", "53/7", "4", " NaN" и "0".
 */
...
public final class RatNum extends Number implements Comparable<RatNum> {
    ...
    /** Константа содержащая значение НЕОПРЕДЕЛЕННОСТИ типа RatNum */
     ...
     */
    //Возвращает некое целое число, знаменатель отбрасываем
    public RatNum(int n) {
        numer = n;
        denom = 1;
        checkRep();
     ...
     */
    //Создает некое рациональное число - в конце проверка на непротиворечивость
    ...
    /**
     * Проверяет значение на непротиворечивость(если такая есть) Checks that the representation invariant holds (if any).
     **/
    ...
    /** Возвращает наибольший общий делитель для чисел 'a' и 'b'.
     @param a, b числа для которых нужно найти НОД - GCD.
     @requires b != 0
