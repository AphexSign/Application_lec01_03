...
public final class RatPoly {
    ...
    public RatPoly(int c, int e) {
        ...
        //Создание конструктора одночлена - с исходной степенью и значением коэффициента
        //В рамках решения задач, не меняя окружающих условий - принимаем остальные члены за нули
        //Пример: 3*x^4+0+0+0+0;
        RatNum[] myRatNum;
        if (!(c == 0)) {
            myRatNum = new RatNum[e + 1];
            int tmpValue = 0;
            for (int i = 0; i < (e + 1); i++) {
                if (i == e) {
                    tmpValue = c;
                }
                myRatNum[i] = new RatNum(tmpValue);
            }
            this.coeffs = myRatNum;
            this.degree = e;
        } else {
    ...
    public RatNum getCoeff(int pow) {
        // TODO: Fill in this method, then remove the RuntimeException
        //Найти по данной степени в выражании - сам коэффицент
        checkRep();
    ...
    public boolean isNaN() {
        // TODO: Fill in this method, then remove the RuntimeException
        // Выявить неопределенное значение среди коэффициентов
        for (RatNum ratNum : this.coeffs) {
            if (ratNum.isNaN()) {
                return true;
            }
        }
    ...
    private static void scaleCoeff(RatNum[] arr, RatNum scalar) {
        // TODO: Fill in this method, then remove the RuntimeException
        //Перемножение всех коэффициентов на некоторое рациональное число
    ...
    public RatPoly negate() {
        //Меняем знаки перед каждым членом на обратный

        //Создаем временный массив

        RatNum[] myCoeff = new RatNum[this.coeffs.length];

        for (int i = 0; i < this.coeffs.length; i++) {
            myCoeff[i] = this.coeffs[i];
        }
        //Умножили все значения на -1
        scaleCoeff(myCoeff, new RatNum(-1));
        //Возвращем наш модифицированный RatPoly
        return new RatPoly(myCoeff);
    ...
    public RatPoly add(RatPoly p) {
        ...
        RatNum[] ratNum=new RatNum[(Math.max((this.coeffs.length),(p.coeffs.length)))];

        for (int i = 0; i < ratNum.length; i++) {
            ratNum[i]=new RatNum(0);
        }

        for(int i=0;i<this.coeffs.length;i++){
            ratNum[i]=this.coeffs[i].add(ratNum[i]);
        }
        for(int i=0;i<p.coeffs.length;i++){
            ratNum[i]=p.coeffs[i].add(ratNum[i]);
        }
        return new RatPoly(ratNum);
    ...
    public RatPoly sub(RatPoly p) {
        ...
        RatNum[] ratNum5=new RatNum[(Math.max((this.coeffs.length),(p.coeffs.length)))];
        //System.out.println(ratNum5.length);
        for (int i = 0; i < ratNum5.length; i++) {
            ratNum5[i]=new RatNum(0);
        }

        for(int i=0;i<this.coeffs.length;i++){
            ratNum5[i]=this.coeffs[i].add(ratNum5[i]);
           // System.out.println(ratNum[i]);
        }
        for(int i=0;i<p.coeffs.length;i++){
            ratNum5[i]=ratNum5[i].sub(p.coeffs[i]);
          //  System.out.println(ratNum5[i]);
        }
        return new RatPoly(ratNum5);
    ...
    public RatPoly mul(RatPoly p) {
        ...
        RatNum[] ratNum=new RatNum[this.coeffs.length+p.coeffs.length];
        //Заполняем нулями наш массив
        for (int i = 0; i < ratNum.length; i++) {
            ratNum[i]=new RatNum(0);
        }
            for(int i=0;i<this.coeffs.length;i++){
                for(int j=0;j<p.coeffs.length;j++){
                    ratNum[i+j]=this.coeffs[i].mul(p.coeffs[j]);
                    System.out.println(ratNum[i+j].toString());
                }
            }
   ...
   //     throw new RuntimeException("RatPoly->mul() is not yet implemented");
    ...
    public double eval(double d) {
        ...
        for (int i = 0; i < this.coeffs.length; i++) {
            result += this.coeffs[i].doubleValue() * Math.pow(d, i);
